...
function readTrack(row: HTMLElement): TrackBox | null {
  const faces = row.querySelectorAll<HTMLElement>(".eq-node-face");
...
function useEquationTrack(rowRef: RefObject<HTMLDivElement | null>) {
  ...
  useLayoutEffect(() => {
    ...
    ro.observe(row);
    row.querySelectorAll(".eq-node-face").forEach((face) => ro.observe(face));
...
function EquationTrack({ track }: { track: TrackBox | null }) {
...
function PlayEquationRow({
  ...
  onFinale,
  shape = "square",
}: Omit<EquationRowProps, "shape"> & { shape?: "square" | "circle" }) {
  ...
  const track = useEquationTrack(ref);
  const circle = shape === "circle";
  const centerShine = circle ? activeCount >= 3 : finale;

  return (
    <div
      ref={ref}
      className={cn(ROW_GAP, circle && "eq-row--circle", className)}
      data-eq-axis={track?.axis ?? "y"}
      style={{ "--p": "0%" } as CSSProperties}
      role="group"
      aria-label={`${a.label} plus ${b.label} equals ${c.label}`}
    >
      <EquationTrack track={track} />
      {circle ? (
        <>
          <CircleNode item={a} variant="default" shine={false} />
          <Operator glyph="+" index={2} circled active={activeCount >= 2} />
          <CircleNode item={b} variant="result" shine={centerShine} />
          <Operator glyph="=" index={4} circled active={activeCount >= 4} />
          <CircleNode item={c} variant="default" shine={false} />
        </>
      ) : (
        <>
          <SquareNode
